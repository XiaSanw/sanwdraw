...
- 在自由画布中添加、移动和编辑硬件组件。
- 为组件编辑名称、描述和颜色，增加电源或信号接口，并设置协议、电压、电流、所在边和边缘位置。
- 接口框环绕组件主框；左、上、右、下接口分别从对应外侧出线，换边后会自动等距分布。
- 多个接口可组成共享母线，同时保留每条支路的独立语义。
- 支持拖动共享母线标签（不改变母线形状）、自定义支路拐点，以及单独删除一条支路；手动走线可吸附为平行或 90°。
- 组件、母线和单条支路均可自定义颜色；支路可恢复继承母线颜色。
- 支持添加文字和图片。
- 选择工具支持矩形框选多个对象；按住 `Shift` 可追加选择，按住 `Cmd/Ctrl` 单击可取消或追加单个对象，拖动任意已选对象可整体移动，按 `Delete` 可一起删除。
- 器件栏可调整宽度、收起和展开。
...
- 单击左侧器件库卡片会在右侧编辑模板；拖动卡片或点击卡片右侧加号，才会把它添加到画布。
- 器件库模板可编辑名称、颜色和尺寸，并分别定义电源口、信号口数量及每个端口的属性；也可通过右侧属性栏顶部的删除图标移除，且不会影响已放置的画布器件。
- 选择工具下从画布空白处拖出矩形即可框选对象；按住 `Shift` 可追加框选，拖动任意已选对象会整体移动。
- 在画布中按 `Cmd/Ctrl + A` 可全选画布对象，不会选中属性栏等界面内容。
- 新增画布器件默认隐藏全部模板端口；选中器件后，在右侧按需启用端口，画布只显示已启用端口。
- 选中画布器件后，可在右侧编辑实例属性；右上角删除图标用于移除画布实例及其相关支路。
- 点击接口框会在右侧打开接口属性；直接拖动接口框即可调整位置，接触同侧接口时邻近接口会自动避让并平滑过渡，换边后接口自动等距分布。
- 只有点击接口框边缘的圆形触点才会开始连线；点击接口框本身不会触发连线。
- 接口换边、调整边缘位置或等距重排时，仅受影响支路会清除旧拐点并重新自动走线，其他支路与母线汇合点保持不变。
- 自动走线会先沿接口朝外引出一小段，再通过正交寻路绕开模块主体和已启用的接口框；线路会保留安全间距，并优先选择较短、拐点较少的路径。
- 拖动模块时，自动母线中心和既有走线通道会尽量保持稳定；必须换路时使用短时路径形变动画，避免线路在上下通道之间快速跳变。
- 将新接口连接到已有线路，可把它吸附到共享母线。
- 点击线段选择单条支路；点击汇合点选择整条母线。
- 共享母线名称标签可以直接拖动并吸附到母线，标签位置不会改变母线形状。
